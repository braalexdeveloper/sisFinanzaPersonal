...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
 ...
 ingreso_month:number=0;
  ...
  ngOnInit(): void {
    this.setData();
    this._authService.user$.subscribe(el=>{
      if(el){
        this._transactionService.getExpenses(el.id).subscribe({
          next:(data)=>{
            this.expense_daily=data.daily_expense.toFixed(2)
            this.expense_month=data.monthly_expense.toFixed(2)
            this.expense_year=data.yearly_expense.toFixed(2)
          },
          error:(error)=>{
            console.log(error)
          }
